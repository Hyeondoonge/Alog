...
const formatDate = (date) => {
  ...
  if (today.diff(writeDate, 'years')) {
    return writeDate.format('YYYY년 MM월 DD일 H:mm');
  }
   
  if (today.diff(writeDate, 'months') || today.diff(writeDate, 'days')) {
    return writeDate.format('MM월 DD일 H:mm');
  }

  return writeDate.format('H:mm');
};

/**
 * doc 타입의 요소들에 대해 값을 조작
 * @param {*} docs Mongoose Document 타입 배열
 * @param {*} callback 각 요소에 수행할 함수 
